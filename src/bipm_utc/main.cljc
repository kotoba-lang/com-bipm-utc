(ns bipm_utc.main
  "Kotodama WASM entrypoint for the bipm_utc clean-room actor (L4) — Clojure port.
  ...
  facts (`bipm_utc.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "bipm_utc")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"bipm_utc.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bipm_utc-compat" :tier tier :entities entities} 200])
...
(defn start! [] :bipm_utc-compat/ready)
